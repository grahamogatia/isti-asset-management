...
function DisplayEmployee({ employee }: { employee: Employee }) {
  ...
  const companyData = companies?.find((c) => c.company_id === employee.company_id);
  ...
  if (combinedBreadcrumb.length > maxTotalLength) {
    ...
    if (availableForPosition > 10) {
      // Ensure we have reasonable space for position
      const truncatedPosition =
        employee.position.substring(0, availableForPosition - 3) + "...";
      displayBreadcrumb = orgBreadcrumb + " › " + truncatedPosition;
    } else {
      // If org path is too long, truncate the whole thing
      displayBreadcrumb =
        combinedBreadcrumb.substring(0, maxTotalLength - 3) + "...";
    }
  } else {
